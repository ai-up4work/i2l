      ...
      <header className="fixed top-0 left-0 w-full z-50">
        ...
        <div className="relative w-full" style={{ height: OUTER_H }}>
          ...
          <div className="relative z-10 container mx-auto px-4 flex items-center justify-between w-full max-w-[1600px] h-full">
            ...
            <div className="flex items-center gap-3 flex-shrink-0 z-20 pr-6 h-full">
              {showBackButton ? (
            ...
            <div className="hidden lg:flex items-center justify-center min-w-0 px-5 absolute left-1/2 top-0 -translate-x-1/2 z-20" style={{ height: INNER_H }}>
              <nav className="flex items-center gap-6 -mt-2" aria-label="Primary navigation">
                {navLinks.map((link) => {
                  ...
                  return (
                    <div key={link.href} className="relative h-full flex items-center"
                      onMouseEnter={() => hasDropdown && setActiveDesktopMenu(link.href)}
                      onMouseLeave={() => hasDropdown && setActiveDesktopMenu(null)}>
                      <a href={link.href} className="group relative flex items-center gap-1 text-sm font-semibold font-body tracking-wide text-ink/85 transition-colors duration-200 hover:text-teal-deep">
                        {link.label}
                        {hasDropdown && (
