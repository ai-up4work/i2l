  ...
  return (
    ...
    >
      <div className="relative p-5">
        <div className="absolute right-5 top-5 flex flex-wrap justify-end gap-1.5">
          <span
            className={`w-max rounded-md px-2.5 py-1 text-[11px] font-semibold ${
              disabled ? 'bg-ink/5 text-ink/40' : 'bg-gold-soft text-rust'
            }`}
          >
            {tag}
          </span>
        ...
        <p className={`max-w-[70%] font-display text-3xl leading-none ${disabled ? 'text-ink/30' : 'text-rust'}`}>
          {discount}
        </p>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-4">
          <span className={`text-xs ${disabled ? 'text-ink/30' : 'text-muted'}`}>Use by {expiresOn}</span>
          {onInfo && (
            <button
              type="button"
              onClick={onInfo}
      ...
      <div
        className={`relative border-t border-dashed before:absolute before:-left-3 before:-top-3 before:h-6 before:w-6 before:rounded-full before:bg-paper before:content-[''] after:absolute after:-right-3 after:-top-3 after:h-6 after:w-6 after:rounded-full after:bg-paper after:content-[''] ${
          disabled ? 'border-ink/15' : 'border-rust/30'
        }`}
      >
        <div className="flex items-center justify-between gap-4 px-5 py-4">
          ...
          <button
            ...
            onClick={handleCopy}
            disabled={disabled}
            className="flex flex-none items-center gap-1.5 rounded-lg bg-rust px-4 py-2 text-xs font-bold text-paper transition-colors enabled:hover:bg-rust/90 disabled:cursor-not-allowed disabled:bg-ink/10 disabled:text-ink/30"
          >
