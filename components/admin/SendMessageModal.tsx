...
// it as-is, edit the wording, attach/replace an image, or skip sending
// entirely. The underlying action (setQuote, confirmPayment,
// flagUnavailable, ...) has already happened by the time this shows —
// this only ever controls whether/what gets said in chat about it,
// never gates the action itself.
//
...
// send, skip, attach, loading/error state) instead of several slightly
// different copies drifting apart over time.
...
export type SendMessageModalProps = {
  ...
  /** Optional image already relevant to this message — e.g. the QC
   * photo just taken when a defect is flagged. Pre-fills the attachment
   * so the admin doesn't have to re-upload something that was already
   * captured moments ago; they can still remove or replace it. Omit
   * entirely for triggers with no natural image (quote, payment, etc.)
   * — the "Attach image" option is still available either way. */
  defaultAttachmentUrl?: string
  /** Called with the (possibly edited) text and an optional attachment
   * URL when the admin clicks Send. Return { ok: false, error } (or
   * reject) if the real send failed — the modal stays open and shows
   * the error instead of closing, so a failure is never silently
   * indistinguishable from success. */
  onSend: (text: string, attachmentUrl?: string) => Promise<{ ok: boolean; error?: string }> | void | Promise<void>
  /** Called when the admin dismisses without sending — closes the modal, does nothing else. */
  onSkip: () => void
}

export function SendMessageModal({ open, title, defaultMessage, defaultAttachmentUrl, onSend, onSkip }: SendMessageModalProps) {
  const [text, setText] = useState(defaultMessage)
  const [attachmentUrl, setAttachmentUrl] = useState<string | undefined>(defaultAttachmentUrl)
  const [sending, setSending] = useState(false)
  const [sendError, setSendError] = useState<string | null>(null)
  const { uploading, error: uploadError, upload } = useImageUpload()
  const fileInputRef = useRef<HTMLInputElement>(null)

  // Re-seed the draft (and attachment) whenever a new message comes in
  // while the modal is open (or reopens) — without this, editing one
  // message and later triggering a different one would show stale
  // leftover text/image.
  useEffect(() => {
    if (open) {
      setText(defaultMessage)
      setAttachmentUrl(defaultAttachmentUrl)
      setSendError(null)
    }
  }, [open, defaultMessage, defaultAttachmentUrl])
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // ok/error — but every current caller does return one, since
      // silently assuming success is exactly the bug this modal exists
      // to prevent.
        ...
        <div className="px-5 py-4">
          ...
          {attachmentUrl ? (
            <div className="mt-3 flex items-start gap-3">
              <img src={attachmentUrl} alt="" className="h-16 w-16 flex-none rounded-lg border border-ink/10 object-cover" />
              <div className="flex flex-col gap-1.5">
                <button
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
                  disabled={uploading || sending}
                  className="flex items-center gap-1.5 text-xs font-semibold text-teal-deep hover:underline disabled:opacity-60"
                >
                ...
                <button
                  ...
                  disabled={sending}
                  className="text-xs font-semibold text-ink/45 hover:text-ink disabled:opacity-60"
                >
                  Remove
                </button>
              </div>
            ...
            <button
              ...
              onClick={() => fileInputRef.current?.click()}
              disabled={uploading || sending}
              className="mt-3 flex items-center gap-1.5 rounded-lg border border-ink/15 bg-white px-3 py-1.5 text-xs font-semibold text-ink/70 hover:bg-parchment/60 disabled:opacity-60"
            >
              {uploading ? <Loader2 size={12} className="animate-spin" /> : <ImagePlus size={12} />}
              {uploading ? 'Uploading…' : 'Attach image'}
            </button>
          )}
          {uploadError && <p className="mt-1.5 text-xs text-red-700">{uploadError}</p>}
