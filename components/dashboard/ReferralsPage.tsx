...
export default function ReferralsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 pb-16 pt-8 lg:px-10">
      <h1 className="text-4xl font-extrabold tracking-tight text-gray-900">Invite &amp; earn</h1>
      ...
      <div className="mt-8 rounded-2xl bg-gold-soft px-8 py-9 motion-safe:[animation:fadeUp_0.4s_ease-out_both]">
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row">
          ...
          <button
            onClick={copyLink}
            className={`flex items-center justify-center gap-2 rounded-xl px-6 py-3.5 text-sm font-semibold text-paper transition-all duration-200 active:scale-95 ${
              copied ? 'bg-rust' : 'bg-ink hover:bg-blue-deep'
            }`}
          >
            {copied ? (
              <>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {['WhatsApp', 'Facebook', 'X', 'Email'].map((channel) => (
            <button
              key={channel}
              className="flex items-center gap-2 rounded-full border border-ink/15 bg-paper px-4 py-2 text-sm font-semibold text-ink transition-all duration-200 hover:-translate-y-0.5 hover:bg-ink/5 hover:shadow-sm active:scale-95"
            >
      ...
      <div className="mt-5 grid gap-5 sm:grid-cols-3">
        {rewardTiers.map((tier, i) => (
          <div
            key={tier.friends}
            className="rounded-2xl border border-dashed border-ink/20 bg-card p-6 transition-all duration-300 hover:-translate-y-0.5 hover:border-rust/40 hover:shadow-md hover:shadow-ink/5 motion-safe:[animation:fadeUp_0.4s_ease-out_both]"
...
function Stat({ label, value, delay }: { label: string; value: number; delay: number }) {
  ...
  return (
    <div
      className="rounded-2xl border border-ink/10 bg-card p-6 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:shadow-ink/5 motion-safe:[animation:fadeUp_0.4s_ease-out_both]"
