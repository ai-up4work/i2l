...
// /components/dashboard/RequestPreviewPage.tsx
'use client'
...
export default function RequestPreviewPage({ draft, promoCode, setPromoCode, onCancel, onNext }: RequestPreviewPageProps) {
  const { subtotal, serviceFee, estimatedFeeUsd, estimatedTotalLkr } = calculateQuote({
    unitPrice: draft.unitPrice,
    qty: draft.qty,
  })
  const estimatedTotalLkr = estimatedFeeUsd * 300
      ...
      <div className="mt-8 flex flex-col gap-6 lg:flex-row lg:items-start">
        <div className="flex min-w-0 max-w-2xl flex-1 flex-col gap-5">
              ...
              <div className="text-right">
                ...
                {draft.url && (
                  <a
                    ...
                    className="mt-2 flex items-center justify-end gap-1 text-xs font-bold uppercase tracking-wide text-teal-deep hover:underline"
                  >
          ...
          <Card icon={Ticket} title="Promo Code">
            <div className="flex gap-2.5 pt-3">
              ...
              <button
                ...
                className="rounded-xl bg-ink px-6 font-semibold text-parchment transition-colors hover:bg-indigo-deep disabled:cursor-not-allowed disabled:bg-ink/25"
              >
                Use
              </button>
            </div>
          ...
          <Card icon={Wallet} title="Fee Details" onEdit={() => {}} editLabel="Edit fee details">
            ...
            <div className="mt-1.5 rounded-xl bg-gold/10 p-4">
              <div className="flex items-center justify-between font-display text-lg text-ink">
                <span className="flex items-center gap-1.5 text-sm font-bold">
                  Estimated Total <CircleHelp size={13} className="text-ink/45" />
                </span>
                <strong>LKR {estimatedTotalLkr.toLocaleString()}</strong>
              </div>
              <small className="mt-2 flex items-center gap-1.5 text-xs text-ink/45">
                Exchange rate <CircleHelp size={12} /> {draft.currency} 1 &asymp; LKR {(estimatedTotalLkr / estimatedFeeUsd).toLocaleString(undefined, { maximumFractionDigits: 2 })}
              </small>
            </div>

            <div className="mt-4 rounded-xl bg-gold/5 p-4">
          ...
          <div className="flex items-center justify-end gap-6 px-1 py-2">
            ...
            <button
              type="button"
              onClick={onNext}
              className="rounded-xl bg-ink px-7 py-3 text-sm font-semibold text-parchment transition-colors hover:bg-indigo-deep"
            >
              Next
            </button>
          </div>
...
function Row({ label, icon: Icon, children }: { label: string; icon: typeof FileText; children: React.ReactNode }) {
  return (
    <div className="flex justify-between gap-5 border-b border-ink/10 py-3">
      <span className="flex flex-none items-center gap-1.5 text-sm text-ink/45">
        <Icon size={14} /> {label}
      </span>
...
function PlainRow({ label, value, muted }: { label: string; value: string; muted?: boolean }) {
  return (
    <div className="flex items-center justify-between border-b border-ink/10 py-3 last:border-0">
      <span className={`text-sm ${muted ? 'text-ink/45' : 'text-ink/70'}`}>{label}</span>
      <b className={`text-sm ${muted ? 'font-medium text-ink/45' : 'font-semibold text-ink'}`}>{value}</b>
    </div>
  )
}
...
export default function RequestPreviewPage({ draft, promoCode, setPromoCode, onCancel, onNext }: RequestPreviewPageProps) {
  // NOTE: `deliveryType` isn't on the `Draft` type yet — defaulting to
  // "express" here until that field exists on Draft and can be threaded
  // through. Same for `weightKg`: left unset so calculateSimpleQuote
  // falls back to its default weight-block size. Wire both up from
  // `draft` once they're available.
  const { subtotal, serviceFee, estimatedFeeUsd, estimatedTotalLkr } = calculateRequestPreviewQuote({
    unitPrice: draft.unitPrice,
    qty: draft.qty,
      ...
      <div className="mt-8 flex flex-col gap-6 lg:flex-row lg:items-start">
        <div className="flex min-w-0 max-w-2xl flex-1 flex-col gap-5">
          ...
          <Card icon={Wallet} title="Fee Details" onEdit={() => {}} editLabel="Edit fee details">
            <PlainRow label={`Subtotal (${draft.qty} pcs)`} value={`${draft.currency} ${subtotal.toFixed(2)}`} muted />
            <PlainRow label="Service fee" value={`${draft.currency} ${serviceFee.toFixed(2)}`} muted />
            <TotalRow label="Estimated Fee" value={`${draft.currency} ${estimatedFeeUsd.toFixed(2)}`} />
