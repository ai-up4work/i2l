  ...
  return (
    <div className="mx-auto max-w-5xl px-6 pb-16 pt-8 lg:px-10">
      <div className="motion-safe:[animation:fadeUp_0.4s_ease-out_both]">
        <h1 className="font-display text-4xl font-extrabold tracking-tight text-ink">My credits</h1>
      ...
      <div className="relative mt-8 flex flex-col gap-6 overflow-hidden rounded-2xl bg-gold/10 px-8 py-9 sm:flex-row sm:items-center sm:justify-between motion-safe:[animation:fadeUp_0.4s_ease-out_0.05s_both]">
        <div
        ...
        <div className="relative">
          <span className="flex items-center gap-2 text-sm font-semibold text-ink/70">
            <Wallet size={16} className="text-gold-deep" /> Available balance
          </span>
        ...
        <a
          href="#earn"
          className="group relative flex items-center justify-center gap-2 rounded-xl bg-ink px-6 py-3 text-sm font-semibold text-parchment transition-all duration-200 hover:bg-teal-deep hover:shadow-md active:scale-95"
        >
      ...
      <div className="mt-5 grid gap-5 sm:grid-cols-3">
        {earnMethods.map((method, i) => (
          <div
            key={method.title}
            className="flex flex-col rounded-2xl border border-dashed border-ink/20 bg-card p-6 transition-colors duration-300 hover:border-gold-deep/40 hover:bg-ink/[0.015] motion-safe:[animation:fadeUp_0.4s_ease-out_both]"
            style={{ animationDelay: `${i * 80}ms` }}
          >
            <span className="w-max rounded-full bg-gold/10 px-3 py-1 text-xs font-bold text-gold-deep">{method.reward}</span>
        ...
        {creditTransactions.length === 0 ? (
          <div className="flex flex-col items-center rounded-2xl border border-dashed border-ink/25 px-6 py-14 text-center motion-safe:[animation:fadeUp_0.4s_ease-out_0.1s_both]">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-ink/5 motion-safe:[animation:scaleIn_0.4s_ease-out_0.05s_both]">
              <Clock3 size={22} className="text-ink/40" />
            </div>
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`grid h-9 w-9 flex-none place-items-center rounded-full border-2 border-dashed ${
                        isEarned ? 'border-gold-deep/40 text-gold-deep' : 'border-ink/25 text-ink/50'
                      }`}
                    >
                    ...
                    <div>
                      ...
                      <span className="text-xs text-ink/40">{transaction.date}</span>
                    </div>
                  </div>
                  <b className={`font-display text-lg tabular-nums ${isEarned ? 'text-gold-deep' : 'text-ink/60'}`}>
                    {isEarned ? '+' : ''}
                    {transaction.amount}
                  </b>
                </div>
