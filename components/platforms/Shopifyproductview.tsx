 ...
 * SAME structural layout as AmazonProductView — gallery + buy box side
 * by side (max-w-6xl), main image with a thumbnail strip beneath it,
 * buy box ordered as badge/title -> seller -> rating -> price/discount
 * -> stock -> variant rows -> original listing link ->
 * ShopifyCommerceActions, then a bottom-most full-width
 * ProductInfoTabs section — but using the app's own design tokens
 * (teal/ink/indigo-deep/gold-deep) rather than imitating a fake
 * "Shopify look": unlike Amazon/Flipkart/etc., Shopify stores have no
 * single canonical storefront skin to match, since every merchant
 * re-themes their own store completely. What IS consistent and worth
 * surfacing is that this data came straight from Shopify's own product
...
function VariantRow({
  ...
  return (
    <div>
      <p className="text-[10px] font-bold uppercase tracking-wide mt-4 text-ink/45">
        {dim.dimension}
...
/**
 * Shopify-styled qty/wishlist/cart/request block — same functional
 * shape as AmazonCommerceActions (qty stepper, wishlist heart, Add to
 * Cart, Get Quote, all inline in one wrapping row), using the app's own
 * teal-deep tokens since there's no Shopify-specific brand color to
 * imitate here (see doc comment above).
 */
    ...
    <div className="mt-6 flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          disabled={loading || result.unavailable || !canAct}
          className="flex flex-1 items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-teal-deep px-5 py-3 text-sm font-semibold text-white transition-all duration-200 hover:bg-indigo-deep hover:shadow-md active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-ink/25"
        >
