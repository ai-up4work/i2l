...
export default function SalesDashboardPage() {
  ...
  const { role, currentUser, visibleOrders, visiblePurchaseLines } = useAdminData()

  useEffect(() => {
    // Effective `role` (previewRole-aware), not currentUser.role — see
    // the same fix/comment on manager-dashboard/page.tsx.
    if (role !== "sales") router.replace("/admin/dashboard")
  }, [role, router])
