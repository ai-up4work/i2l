...
export default function CatalogueDetailPage() {
  ...
  useEffect(() => {
    // Admin API (staff-gated, sees hidden products too) instead of the
    // browser client, which RLS limits to active products only.
    fetch(`/api/admin/catalogues/${params.catalogueId}`)
      .then(async (res) => {
        const body = await res.json().catch(() => ({}))
        const data = res.ok ? (body.product as Product | null) : null
        setProduct(data)
        setMarginInput(data ? String(data.margin_percent ?? 25) : '')
      })
