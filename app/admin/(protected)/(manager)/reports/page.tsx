...
export default function ReportsPage() {
  ...
  const { role, currentUser, orders, purchases, sites } = useAdminData()
  const [range, setRange] = useState<DateRangeOption>("30d")
  const [siteFilter, setSiteFilter] = useState<string>("all")
  const [siteSort, setSiteSort] = useState<SiteSort>("attention")

  useEffect(() => {
    // Effective `role` (previewRole-aware), and lets super_admin through
    // unconditionally even with no preview active — same fix/comment as
    // manager-dashboard/page.tsx. This is the exact guard that made
    // "Super Admin unable to access Manager pages" happen: it used to
    // check currentUser.role, which never equals "manager" for a real
    // super_admin, bouncing them straight back to /admin/dashboard no
    // matter what.
